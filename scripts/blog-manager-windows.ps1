...
$PostDir = Join-Path $BlogDir "src\content\blog"
$AboutFile = Join-Path $BlogDir "src\content\about.md"
...
function Get-ArticleTags {
    ...
    $match = [Regex]::Match(
        $content,
        '(?ms)^tags:\s*(.*?)^(?=[A-Za-z][A-Za-z0-9_-]*:\s*|---\s*$)'
    )
    if (-not $match.Success) {
        return "未分类"
    }

    $clean = $match.Groups[1].Value.Trim().Trim('[', ']')
    $tags = @(
        $clean -split '[,\r\n]+' |
...
function Get-Articles {
    ...
    return @(
        Get-ChildItem -LiteralPath $PostDir -Filter "index.md" -File -Recurse |
...
function Show-Articles {
    ...
    for ($i = 0; $i -lt $articles.Count; $i++) {
        ...
        Write-Host ("    标签：{0} · {1}" -f $tags, $file.Directory.Name)
    }
    return $true
}
...
function Test-Dependencies {
    ...
    if ($null -eq $npm) {
        Write-Host "没有找到 Node.js/npm，无法运行炫光版。"
        Write-Host "请先安装 Node.js：https://nodejs.org/"
        return $false
    }

    if (-not (Test-Path -LiteralPath (Join-Path $BlogDir "node_modules"))) {
        Write-Host "首次运行，正在安装依赖……"
        & npm.cmd install
        if ($LASTEXITCODE -ne 0) {
            Write-Host "依赖安装失败，请检查网络和上面的 npm 提示。"
            return $false
        }
    }
    return $true
...
function New-Article {
    ...
    $articleDir = Join-Path $PostDir $slug
    $file = Join-Path $articleDir "index.md"
    if (Test-Path -LiteralPath $articleDir) {
        Write-Host ("英文简称已经存在：{0}" -f $slug)
        Pause-Menu
        return
    }
    [void](New-Item -ItemType Directory -Path $articleDir)

    $lines = @(
        "---",
        ('title: "' + (Escape-Yaml $title) + '"'),
        ('description: "' + (Escape-Yaml $description) + '"'),
        ('publishDate: ' + (Get-Date).ToString("yyyy-MM-ddTHH:mm:sszzz")),
    ...
    Write-Host ""
    Write-Host ("已创建草稿：{0}\index.md" -f $slug)
...
function Toggle-Article {
    ...
    if ($state -eq "草稿") {
        ...
        if ($answer -match '^(y|yes)$') {
            ...
            Write-Host "已经设为发布状态。运行构建/发布后，文章会出现在网站上。"
        } else {
            Write-Host "没有修改。"
        }
    } else {
        ...
        if ($answer -match '^(y|yes)$') {
            ...
            Write-Utf8File -Path $article.FullName -Content $content
            Write-Host "已经隐藏为草稿。运行构建/发布后，网站上将不再显示它。"
        } else {
            Write-Host "没有修改。"
        }
    }
...
function Delete-Article {
    ...
    Write-Host ""
    Write-Host ("准备把整篇文章移到回收站：{0}" -f (Get-ArticleTitle $article.FullName))
    Write-Host "文章目录里的配图也会一起移动；发布后原网址会变成 404。"
    ...
    [Microsoft.VisualBasic.FileIO.FileSystem]::DeleteDirectory(
        $article.Directory.FullName,
        [Microsoft.VisualBasic.FileIO.UIOption]::OnlyErrorDialogs,
        [Microsoft.VisualBasic.FileIO.RecycleOption]::SendToRecycleBin
    )
    Write-Host "文章已经移到回收站。运行构建/发布后，线上文章才会删除。"
    Pause-Menu
}
...
function Preview-Site {
    ...
    $preview = Start-Process -FilePath "npm.cmd" -ArgumentList $arguments -WorkingDirectory $BlogDir -PassThru -WindowStyle Hidden
    Start-Sleep -Seconds 2
    $preview.Refresh()
    if ($preview.HasExited) {
        Write-Host "预览启动失败，请确认 Node.js 和依赖已正确安装。"
        Pause-Menu
        return
    }
...
function Build-Site {
    ...
    Write-Host "正在检查并构建网站……"
    & npm.cmd run build
    if ($LASTEXITCODE -ne 0) {
        Write-Host "构建失败，请根据上面的错误信息修改内容。"
        return $false
    }

    Write-Host ("网站构建成功，静态文件在：{0}" -f (Join-Path $BlogDir "dist"))
    return $true
}
...
function Publish-Site {
    ...
    if ($null -eq (Get-Command "git" -ErrorAction SilentlyContinue)) {
        Write-Host "本地构建已完成，但没有找到 Git，所以没有发布。"
        Pause-Menu
        return
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "本地构建已完成。"
        Write-Host "这个独立版本尚未绑定 Git 仓库，因此不会覆盖另外两个版本或线上博客。"
        Write-Host "需要发布时，可以单独为“炫光版”配置 GitHub 仓库。"
        Pause-Menu
        return
    }
    & git remote get-url origin *> $null
    if ($LASTEXITCODE -ne 0) {
        Write-Host "本地构建已完成，但没有 origin 远端，因此没有发布。"
        Pause-Menu
        return
    }
    ...
    if ($branch -ne "main") {
        Write-Host ("没有发布：当前分支是“{0}”，请切换到 main。" -f $branch)
        Pause-Menu
        return
    }

    Write-Host ""
    Write-Host "本次检测到的文件变化："
    $changes = @(& git -c core.quotepath=false status --short)
    if ($changes.Count -eq 0) {
        Write-Host "没有新的文件变化，将尝试推送尚未发布的提交。"
        & git push origin main
        if ($LASTEXITCODE -eq 0) {
            Write-Host "发布完成。GitHub Pages 稍后会自动更新网站。"
        } else {
            Write-Host "推送失败，请检查网络或上面的 Git 提示。"
        }
        Pause-Menu
        return
    }
    ...
    if ([string]::IsNullOrWhiteSpace($message)) {
        $message = "更新炫光版博客 " + (Get-Date -Format "yyyy-MM-dd HH:mm")
    }

    & git add -A
    if ($LASTEXITCODE -ne 0) {
        Write-Host "无法把文件加入本次提交。"
        Pause-Menu
        return
    }
    & git diff --cached --quiet
    if ($LASTEXITCODE -eq 0) {
        Write-Host "没有可以提交的文件。"
        Pause-Menu
        return
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "本地提交已经保存，但推送失败；修复网络后再次运行即可继续推送。"
    }
    Pause-Menu
}
...
function Start-BlogManager {
    while ($true) {
        ...
        $repoStatus = "独立本地版本（尚未绑定 Git 仓库）"
        if ($null -ne $git) {
            & git rev-parse --is-inside-work-tree *> $null
        }
        if ($null -ne $git -and $LASTEXITCODE -eq 0) {
            $pendingCount = @(& git status --porcelain 2>$null).Count
            $repoStatus = "Git 中有 $pendingCount 个尚未提交的文件变化"
        }

        Write-Host "================================"
        Write-Host "      小龙的炫光版博客管理工具"
        Write-Host "           Windows 版"
        Write-Host "================================"
        Write-Host $repoStatus
        Write-Host ""
        Write-Host "1. 新建文章"
        Write-Host "2. 查看文章列表"
        Write-Host "3. 编辑文章"
